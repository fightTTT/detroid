...
struct Material {
	...
	Material(const Color& diff,
			 ...
			 float inSpecularity,
			 float inReflectance,
			 Pattern p) : diffuse(diff),/* specular(spec), ambient(amb),*/
...
public:
	...
	virtual Position3 position()const;

	virtual Color specDefCol(const Material& material, const Color& albed,const Vector3& lite, const Position3& hitPos, const Vector3& normal, const Position3& eye);
